...
import static com.swecor.aerotek.service.Constants.COOLER_ID;

@Service
@Transactional
public class CoolerSelectionServiceImpl implements HeatExchangerSelectionService {
    ...
    public CoolerSelectionServiceImpl(RoenEstHeatExchangerRepository heatExchangerRepository, VariableElementRepository variableElementRepository, RoenEstClient roenEstClient) {
    ...
    public SectionToConfigurator selectHeatExchanger(int flowConsumption, byte standardSize, SectionToConfigurator sectionToConfigurator) {
        ...
        List<VariableElement> variableElementList = variableElementRepository.getByStandardSizeAndSectionTypeIdOrderByOrderKeyAsc(standardSize, COOLER_ID);
        if (variableElementList.isEmpty()) {
            throw new ListOfVariableElementsIsEmpty("Водяной охладитель");
        }
        ...
        for (VariableElement variableElement : variableElementList) {
            ...
            RoenEstHeatExchanger roenEstHeatExchangerDB = heatExchangerRepository.findByModeAndCode((byte) 2, variableElement.getIndex()).get(0);
