...
@Service
@Transactional
public class HeaterCalculationService implements Calculated {

    private final HeatExchangerSelectionService heatExchangerSelectionService;

    private final SectionRepository sectionRepository;

    public HeaterCalculationService(@Qualifier("heaterSelectionServiceImpl") HeatExchangerSelectionService heatExchangerSelectionService, SectionRepository sectionRepository) {
    ...
    private void calculateFlowParameters(int flowConsumption, List<SectionToConfigurator> flowSections, byte standardSize) {
        ...
        for (SectionToConfigurator sectionToConfigurator : flowSections) {
            if (sectionToConfigurator.getSectionType().getId() == HEATER_ID) {
