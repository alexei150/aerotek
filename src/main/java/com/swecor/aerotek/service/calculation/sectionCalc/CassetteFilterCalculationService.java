...
@Service
@Transactional
public class CassetteFilterCalculationService implements Calculated {
    ...
    public CassetteFilterCalculationService(AirSpeedService airSpeedService, PressureLossService pressureLossService1, SectionRepository sectionRepository) {
    ...
    private void calculateFlowParameters(List<SectionToConfigurator> flowParameters, int flowConsumption, byte standardSize) {
        ...
        for (SectionToConfigurator sectionToConfigurator : flowParameters) {
            if (sectionToConfigurator.getSectionType().getId() == CASSETTE_FILTER_ID) {

                Section selectedSection = sectionRepository.getCassetteFilter(standardSize, CASSETTE_FILTER_ID, getFilterClass(sectionToConfigurator), FILTER_CLASS_FORM);
        ...
        for (ParameterToSelection parameter : formParameters) {
            if (parameter.getParam().getName().equals(FILTER_CLASS_FORM)) {
                if (parameter.getValue() == null){
                    throw new ParameterValueIsNull("Параметр '" + parameter.getParam().getName() + "' у Кассетного фильтра == null");
                }
                return parameter;
            }
        }
        throw new ParameterByNameNotFound(FILTER_CLASS_FORM + "' у Кассетного фильтра");
    }
}
