        ...
        for (ParameterToSelection parameter : formParameters) {
            if (parameter.getParam().getName().equals(SECTION_LENGTH)) {
                if (parameter.getValue() == null){
                    throw new ParameterValueIsNull("Параметр '" + parameter.getParam().getName() + "' у Пустой секции == null");
                }
                return parameter;
            }
        }
        throw new ParameterByNameNotFound(SECTION_LENGTH + "' у Пустой секции");
    }
}
